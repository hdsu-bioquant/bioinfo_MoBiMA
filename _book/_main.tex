...
\hypertarget{week-1-descriptive-statistics-and-data-types}{%
\chapter{WEEK 1: Descriptive statistics and data types}\label{week-1-descriptive-statistics-and-data-types}}
...
\section{Data features and where to find them}\label{data-features-and-where-to-find-them}}
...
\subsection{Dimensions and naming}\label{dimensions-and-naming}}
...
\hypertarget{accross}{%
\subsection{accross()}\label{accross}}
...
\subsection{Ready for the cleaning!}\label{ready-for-the-cleaning}}
...
\section{Visualizing data distribution}\label{visualizing-data-distribution}}
...
\FunctionTok{ggplot}\NormalTok{(dat,}
  ...
  \FunctionTok{geom\_qq\_line}\NormalTok{()                  }\CommentTok{\# adds in the QQ{-}line on top}
\end{Highlighting}
\end{Shaded}
...
\subsection{Measuring the centrality in data}\label{measuring-the-centrality-in-data}}
...
\subsection{Association between variables}\label{association-between-variables}}
...
\subsection{Exercise 1: Data features}\label{exercise-1-data-features}}
...
\subsection{Exercise 2: Visualization}\label{exercise-2-visualization}}
...
\subsection{Exercise 3: Correlation}\label{exercise-3-correlation}}
...
\hypertarget{week-1-dimensionality-reduction-and-unsupervised-learning}{%
\chapter{WEEK 1: Dimensionality reduction and unsupervised learning}\label{week-1-dimensionality-reduction-and-unsupervised-learning}}
...
\subsection{Performing k-means}\label{performing-k-means}}
...
\subsection{Quality of the clustering}\label{quality-of-the-clustering}}
...
\subsection{Determine the most variable genes}\label{determine-the-most-variable-genes}}
...
\subsection{Computing the correlation between all patients}\label{computing-the-correlation-between-all-patients}}
...
\subsection{Including clinical annotations in the heatmap}\label{including-clinical-annotations-in-the-heatmap}}
...
\subsection{Exercise 1: Variance}\label{exercise-1-variance}}
...
\subsection{Exercise 2: Hierarchical clustering}\label{exercise-2-hierarchical-clustering}}
...
\subsection{Exercise 3: PCA}\label{exercise-3-pca}}
...
\subsection{\texorpdfstring{Going further \emph{(expert)}}{Going further (expert)}}\label{going-further-expert}}
...
\hypertarget{week-1-probability-distributions}{%
\chapter{WEEK 1: Probability distributions}\label{week-1-probability-distributions}}
...
\subsection{Getting to know the various functions}\label{getting-to-know-the-various-functions}}
...
\FunctionTok{plot}\NormalTok{(x, y, }\AttributeTok{type=}\StringTok{\textquotesingle{}l\textquotesingle{}}\NormalTok{,}\AttributeTok{col=}\StringTok{\textquotesingle{}red\textquotesingle{}}\NormalTok{,}\AttributeTok{lwd=}\DecValTok{2}\NormalTok{);}\FunctionTok{abline}\NormalTok{(}\AttributeTok{v=}\FunctionTok{c}\NormalTok{(}\FloatTok{1.6}\NormalTok{,}\FloatTok{1.9}\NormalTok{),}\AttributeTok{lty=}\DecValTok{3}\NormalTok{,}\AttributeTok{lwd=}\DecValTok{2}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\FunctionTok{hist}\NormalTok{(x,}\AttributeTok{breaks=}\DecValTok{20}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\subsection{Normal/Gaussian distribution}\label{normalgaussian-distribution}}
...
\subsection{Application on a real dataset}\label{application-on-a-real-dataset}}
...
\subsubsection{Data cleaning and central values}\label{data-cleaning-and-central-values}}
...
\subsubsection{Modeling using a normal distribution}\label{modeling-using-a-normal-distribution}}
...
\subsubsection{Data prediction using the normal distribution model}\label{data-prediction-using-the-normal-distribution-model}}
...
\subsubsection{Evaluating the quality of the predictions}\label{evaluating-the-quality-of-the-predictions}}
...
\CommentTok{\# Model and measured data:}
\FunctionTok{plot}\NormalTok{(}\FunctionTok{density}\NormalTok{(tp53.exp}\SpecialCharTok{$}\NormalTok{TP53\_expression), }\AttributeTok{main =}\StringTok{""}\NormalTok{, }\AttributeTok{xlim=}\FunctionTok{c}\NormalTok{(}\SpecialCharTok{{-}}\DecValTok{1500}\NormalTok{, }\DecValTok{6000}\NormalTok{),}\AttributeTok{lwd=}\DecValTok{3}\NormalTok{);}\FunctionTok{lines}\NormalTok{(x,d.pred, }\AttributeTok{col=}\StringTok{"red"}\NormalTok{,}\AttributeTok{lwd=}\DecValTok{3}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\subsubsection{Visualization using a Q-Q plot}\label{visualization-using-a-q-q-plot}}
...
\FunctionTok{ggplot}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{geom\_point}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ q.theoretical, }
                 \AttributeTok{y =}\NormalTok{ q.observed), }\AttributeTok{size =} \DecValTok{1}\NormalTok{) }\SpecialCharTok{+}
...
\subsection{Exercise 1: Probability distributions}\label{exercise-1-probability-distributions}}
...
\subsection{Exercise 2: Binomial distribution}\label{exercise-2-binomial-distribution}}
...
\subsection{Exercise 3: Confidence intervals}\label{exercise-3-confidence-intervals}}
...
\hypertarget{week-1-hypothesis-testing}{%
\chapter{WEEK 1: Hypothesis testing}\label{week-1-hypothesis-testing}}
...
\subsection{Two-sample t-test (2-sided)}\label{two-sample-t-test-2-sided}}
...
\subsection{Two-sample t-test (1-sided)}\label{two-sample-t-test-1-sided}}
...
We haven't check that yet for our dataset \ldots{} This is the topic of the section \textbf{going further} below.
...
\subsection{Exercise 1: One-sided t-test}\label{exercise-1-one-sided-t-test}}
...
\subsection{Exercice 2: Two-sided t-test}\label{exercice-2-two-sided-t-test}}
...
\subsection{Exercise 3. Going further \ldots{} Checking the normality of the distribution}\label{exercise-3.-going-further-checking-the-normality-of-the-distribution}}
...
\hypertarget{week-1-multiple-testing-and-regression}{%
\chapter{WEEK 1: Multiple testing and regression}\label{week-1-multiple-testing-and-regression}}
...
\hypertarget{week-2-annotathon}{%
\chapter{WEEK 2: Annotathon!}\label{week-2-annotathon}}

\hypertarget{week-2-data-formats-and-where-to-find-them}{%
\chapter{WEEK 2: Data formats and where to find them}\label{week-2-data-formats-and-where-to-find-them}}

\hypertarget{week-2-rna-seq---from-fastq-to-count-matrix}{%
\chapter{WEEK 2: RNA-seq - from FASTQ to count matrix}\label{week-2-rna-seq---from-fastq-to-count-matrix}}
