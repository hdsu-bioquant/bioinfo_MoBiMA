...
Check your name, your user name and the name of the server on which you will connect.

#### if you are running machine with Linux or MacOS

-   open a terminal and type the following commant: `ssh <USER>@194.94.113.18 -p <PORT> -L 8787:127.0.0.1:8787`; replace `<USER>` with your user name, and `<PORT>` with the server port from the Google Sheet
...
-   In the main panel:
    ...
    -   Port: <PORT> (check the right port in the Google Sheet)
    -   Connection type: SSH
...
You can also choose to install both R and RStudio.
